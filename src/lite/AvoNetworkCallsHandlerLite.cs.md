...
4. `sendTrackingRequest` sets `Content-Type: application/json`, `api-key`, `env` and `X-Avo-Client`, adds `Content-Encoding: gzip` only when gzipped, applies `AvoInspector.networkTimeout`, POSTs to `trackingEndpoint` (`https://api.avo.app/inspector/v2/track`). `apiKey`/`env` stay in the body too, where v2 ignores them.
...
- **Textual-sync invariant:** the gzip helper, threshold, send path (headers included), `TrackOptions` and `normalizeHint` are byte-for-byte the same logic as `AvoNetworkCallsHandler`; `verify:lite-sync` enforces the allowed drift baseline and `check:lite-size` enforces the bundle budget.
