import type { EventSpec, FetchEventSpecParams } from "./types";
...
export class EventSpecFetcher {
  ...
  constructor(
    ...
    baseUrl: string = "https://api.avo.app/inspector/v1"
  ) {
  ...
  private generateRequestKey(params: FetchEventSpecParams): string {
    ...
    return `${params.schemaId}:${params.sourceId}:${params.eventName}:${branchId}`;
  }
  ...
  async fetch(params: FetchEventSpecParams): Promise<EventSpec | null> {
    ...
    if (existingRequest) {
      if (this.shouldLog) {
        console.log(
          `[EventSpecFetcher] Returning existing in-flight request for: ${requestKey}`
        );
      }
